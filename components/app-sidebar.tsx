...
// Example: Replace static or undefined list with hardcoded list or API response
const langflowAgents = [
  { id: 'classifier', name: 'Classifier' },
  { id: 'email', name: 'Email' },
  { id: 'json', name: 'Json' },
  { id: 'pdf', name: 'Pdf' },
]
{langflowAgents.map((agent) => (
...
export function AppSidebar({ selectedFolder, onFolderSelect, onManageFolders, folders }: AppSidebarProps) {
  ...
  const [langflowAgents, setLangflowAgents] = useState<{ id: string; name: string }[]>([])
        ...
        <SidebarGroup>
          ...
          <SidebarGroupContent>
            <SidebarMenu>
              ...
              {/* LangFlow agents section */}
              <SidebarMenuItem>
                <SidebarMenuButton className="w-full justify-start">
                  <FolderOpen className="w-4 h-4" />
                  <span>LangFlow Agents</span>
                  <Badge variant="secondary" className="ml-auto">
                    {langflowAgents.length}
                  </Badge>
                </SidebarMenuButton>
              </SidebarMenuItem>
              ...
              <SidebarMenuSub>
                {langflowAgents.map((agent) => (
                  <SidebarMenuSubItem key={agent.id}>
                    <SidebarMenuSubButton
                    onClick={() => onFolderSelect(agent.id)}
                    isActive={selectedFolder === agent.id}
                    className="flex items-center justify-between"
                  >
