...
import LangFlowAgentRunner from "@/components/langflow-agent-runner"

// Example rendering all agents
...
export function OrchestrationDashboard() {
  ...
  const agentIds = ["classifier", "email", "json", "pdf"]

  return (
    <SidebarProvider defaultOpen={true}>
      <div className="space-y-6">
        {agentIds.map((agent) => (
          <LangFlowAgentRunner key={agent} agentId={agent} />
        ))}
      </div>
